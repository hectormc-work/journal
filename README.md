...
- Docker (for Postgres)
- piqued binary, pinned at **0.7.12** (Phase 2+ only):
...
yarn install
yarn db:up           # Postgres 17 in Docker (not needed until Phase 2)
...
- `yarn dev` — hand-rolled runner (`scripts/dev.mjs`) spawns both dev servers, prefixed output, dies together
